      ...
      ${sources
        .map((source) => {
          const label = source.publisher
            ? `${escapeHtml(source.title)} â€” ${escapeHtml(source.publisher)}`
...
/* =========================================================
   Responsive Image Optimization (Netlify Image CDN)
   ========================================================= */

function canUseNetlifyImageCdn(image = "") {
  const normalized = normalizeImagePath(image);
  if (!normalized) return false;
...
function netlifyImageUrl(image = "", width = 0, quality = 72) {
  const normalized = displayImagePath(image);
  if (!canUseNetlifyImageCdn(normalized)) return normalized;

  const safeWidth = Math.max(1, Math.round(Number(width) || 960));
  const safeQuality = Math.min(100, Math.max(1, Math.round(Number(quality) || 72)));

  return `/.netlify/images?url=${encodeURIComponent(normalized)}&w=${safeWidth}&q=${safeQuality}`;
}

function responsiveImageSrcset(image = "", widths = [480, 720, 960, 1200], quality = 72) {
  const normalized = displayImagePath(image);
  if (!canUseNetlifyImageCdn(normalized)) return "";

  return widths
    .map((width) => `${netlifyImageUrl(normalized, width, quality)} ${width}w`)
    .join(", ");
}
...
<svg xmlns="http://www.w3.org/2000/svg" width="1200" height="700" viewBox="0 0 1200 700">
  ...
  <text x="600" y="315" text-anchor="middle" font-family="Arial, Helvetica, sans-serif" font-size="92">ًںگ¾</text>
...
function header(
  ...
  const preloadImage = options.preloadImage || null;

  const imagePreloadMarkup = preloadImage && preloadImage.href
    ? `<link rel="preload" as="image" href="${escapeHtml(preloadImage.href)}"${preloadImage.srcset ? ` imagesrcset="${escapeHtml(preloadImage.srcset)}"` : ""}${preloadImage.sizes ? ` imagesizes="${escapeHtml(preloadImage.sizes)}"` : ""} fetchpriority="high">`
    : "";
...
<header class="site-header">
  <div class="site-header-inner">
    ...
    >
      ...
      <a${navClass("dogs")} href="/dogs/"><span aria-hidden="true">ًںگ¾</span> Dogs</a>
      <a${navClass("cats")} href="/cats/"><span aria-hidden="true">ًںگ¾</span> Cats</a>
      ...
      <div class="nav-dropdown">
        <button
          ...
          aria-expanded="false"
        >
          Categories <span aria-hidden="true">âŒ„</span>
        </button>
...
<footer class="site-footer">
  <div class="footer-top">
    <div class="footer-trust-item">
      <span class="footer-trust-icon">â™¢</span>
      <div>
    ...
    <div class="footer-trust-item">
      <span class="footer-trust-icon">âœڑ</span>
      <div>
  ...
  <div class="footer-bottom">
    ...
    <p class="footer-disclaimer">
      آ© ${new Date().getFullYear()} Virixoo. Educational information only; not a substitute for veterinary care.
    </p>
  </div>
</footer>
...
>
  ...
  <div class="article-card-body">
    ...
    <div class="article-card-actions">
      <a class="read-more" href="${url}" aria-label="Read ${escapeHtml(article.title)}">Read guide <span>â†’</span></a>
...
<section class="home-hero hero-reference-layout">
  <div class="hero-decor hero-decor-left" aria-hidden="true">ًںگ¾</div>
  <div class="hero-decor hero-decor-right" aria-hidden="true">ًںگ¾</div>

  <div class="hero-copy">
    <span class="hero-badge"><span aria-hidden="true">ًںگ¾</span> Practical Pet Care Guides</span>
    ...
    <div class="hero-actions">
      <a class="primary-button" href="/dogs/">
        <span aria-hidden="true">ًںگ¾</span> Explore Dog Guides
      </a>
      <a class="secondary-button" href="/cats/">
        <span aria-hidden="true">ًںگ¾</span> Explore Cat Guides
      </a>
    </div>
  </div>
  ...
  <aside class="hero-stats" aria-label="Virixoo guide library">
    ...
    <div class="hero-stat-card">
      <span class="hero-stat-icon">â™ں</span>
      <div>
        <strong>100%</strong>
...
<section class="care-switchboard" aria-label="Browse care guides by pet">
  <a class="care-panel dog-care-panel" href="/dogs/">
    ...
    <div class="care-panel-content">
      <span class="care-panel-icon">ًںگ¾</span>
      <div>
        <h2>Dog Care</h2>
        <p>Training, health, nutrition and behavior guides</p>
        <span class="care-panel-link">Browse Dog Articles <b>â†’</b></span>
      </div>
    </div>
  </a>
  ...
  <a class="care-panel cat-care-panel" href="/cats/">
    <div class="care-panel-content">
      ...
      <div>
        ...
        <span class="care-panel-link">Browse Cat Articles <b>â†’</b></span>
      </div>
    </div>
    <img
  ...
  <div class="section-heading">
    <div>
      <span class="section-title-row">
        <span class="section-title-icon">âک…</span>
        <h2 id="featured-guides">Featured Guides</h2>
      </span>
      <p>Handpicked articles to help you care for your pets better</p>
    </div>
    <a class="section-link" href="/categories/">View all articles <span>â†’</span></a>
  </div>
...
<section class="home-editorial-band">
  ...
  <a href="/about/">About our approach <span>â†’</span></a>
</section>
...
function createArticlePage(article, allArticles) {
  ...
  const image = displayImagePath(article.image || "");
  const articleHeroSizes = "(max-width: 760px) calc(100vw - 32px), (max-width: 1000px) calc(100vw - 48px), 892px";
  const articleHeroSrcset = responsiveImageSrcset(image, [480, 640, 768, 960, 1200], 72);
  const articleHeroSrc = canUseNetlifyImageCdn(image)
    ? netlifyImageUrl(image, 960, 72)
    : image;
  const pinterestUrl = pinterestShareUrl(article);
...
<article class="single-article">
  ...
  <img
    ...
    ${articleHeroSrcset ? `srcset="${escapeHtml(articleHeroSrcset)}"` : ""}
    sizes="${escapeHtml(articleHeroSizes)}"
    alt="${escapeHtml(
      article.alt ||
      article.imageAlt ||
      article.title
...
<section class="articles-section related-section" aria-labelledby="related-guides">
  <div class="section-heading">
    ...
    <a class="section-link" href="/${categorySlug}/">
      View all ${escapeHtml(category)} guides <span>â†’</span>
    </a>
  </div>
...
<section class="category-header category-${slug}-header">
  <div>
    <span class="hero-badge">ًںگ¾ ${escapeHtml(normalizedCategory)} Care</span>
...
<section class="category-header categories-overview-header">
  <div>
    <span class="hero-badge">ًںگ¾ Explore Virixoo</span>
...
<section class="category-overview-grid">
  <a class="category-overview-card dogs-overview-card" href="/dogs/">
    <span class="category-overview-icon">ًںگ•</span>
    <div>
      <h2>Dog Care</h2>
      <p>${dogs.length} guides</p>
      <span>Browse Dog Articles â†’</span>
    </div>
  </a>

  <a class="category-overview-card cats-overview-card" href="/cats/">
    <span class="category-overview-icon">ًںگˆ</span>
    <div>
      <h2>Cat Care</h2>
      <p>${cats.length} guides</p>
      <span>Browse Cat Articles â†’</span>
    </div>
  </a>
</section>
...
<section class="search-page">
  <div class="search-page-header">
    <span class="hero-badge">âŒ• Search Virixoo</span>
...
<section class="search-page error-page" aria-labelledby="not-found-heading">
  <div class="search-page-header">
    <span class="hero-badge">404 آ· Page Not Found</span>
    <h1 id="not-found-heading">We couldnâ€™t find that page</h1>
    <p>
...
function build() {
  ...
  createInfoPage({
    ...
    contentHtml: `
      <header class="article-header">
        <span class="hero-badge">ًںگ¾ About Virixoo</span>
  ...
  createInfoPage({
    ...
    contentHtml: `
      <header class="article-header">
        <span class="hero-badge">ًںگ¾ Our Team</span>
    ...
    contentHtml: `
      ...
      <section>
        ...
        <p>
          ...
          functionality, analytics, preferences, security andâ€”if advertising is
          enabledâ€”ad delivery and measurement.
        </p>
      </section>
